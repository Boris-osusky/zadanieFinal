...
try {
    ...
    if ($task) {
        //vyhodnotenie ci je odpoved spravna (mat.vyhodnotenie||string equal vyhodnotenie)
        if (compareLaTeXMeaning(trim($data), trim($task['answer'])) || strcasecmp(trim($data), trim($task['answer']))) {
